...
- Becoming a parametric CAD program (that is [Draft](https://github.com/jtwolfe/atelier-draft); Seamly2D is a neighbour).
...
6. After calibrate, CalSheets can come off. The lamp draws the board grid from the measured pitch.
...
| Sheets off | Projected grid lands on the physical mat grid after the A4s are removed |
| Remote UI (v1) | Tablet on the bench controls the laptop; webcam is still the laptop’s |

## Product boundaries

Table may **write** IR, layouts, and evaluated `.sm2d`. It may **read** IR, SVG, DXF, PDF. It may **not** grow a cloth solver, a pose estimator, or a live formula CAD.
